...
if (!empty($_REQUEST) && !wp_verify_nonce( @$_REQUEST['wpcsw_token'], 'wpcsw_token' )) wp_die(__('Incorrect usage.'));
...
if ( !empty( $_FILES ) ) {
    ...
    $file_url       = trailingslashit( $upload_dir['baseurl'] ).'copysafe-web/'.$file_name;
    $file_path      = trailingslashit( $upload_dir['basedir'] ).'copysafe-web/'.$file_name;
    ...
    if ( empty( $msg ) ) {
        ...
        foreach ( $wpcsw_options['files_list'] as $key => $file ) {
            if ( $file['name'] == $file_name ) {
                unset ( $wpcsw_options['files_list'][$key] );
            }
        }
        ...
        if ( move_uploaded_file( $file_tmp_name, $file_path ) ) {
            ...
            // sort arrays
            */
            ksort ( $wpcsw_options['files_list'] );
            //ksort ( $wpcsw_options['files_options'] );
...
if (!empty($_POST) && empty($_FILES)) {
    ...
    $capture_safe    = ( empty( $capture_safe ) ? '' : '1' );
    $menu_safe       = ( empty( $menu_safe ) ? '' : '1' );
    $remote_safe     = ( empty( $remote_safe ) ? '' : '1' );
    ...
    if ( $option_key === 0 || $option_key > 0  ) {
        $wpcsw_options['files_options'][$option_key] = array (
                                                        'border'          => "$border",
...
<link rel='stylesheet' href='./wp-copysafe-web.css' type='text/css' media='all' />
        ...
        <div id="wpcontentt">
            <div id="wpbody">
                <div id="wpbody-content">
                    <div class="wrap" id="wpcsw_div" title="CopySafe">
                        $msg
                        <div id="tabs">
                            <ul>
                            ...
                            <div id="tabs-1" class="wpcsw_addnew">
                                ...
                                <form id="file-form" class="media-upload-form" action="" method="post" enctype="multipart/form-data">
                                    ...
                                    <p>
                                        <input type="file" accept=".class" id="file" name="wpcsw_file" />
                            ...
                            <div id="tabs-2" class="wpcsw_search">
                            ...
                            <div id="tabs-3" class="wpcsw_filelist">
